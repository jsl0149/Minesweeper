...
class Level3 extends JFrame{
	...
	public JPanel panelLv2       = new JPanel(new GridLayout(0,30)); //지뢰타일 넣을 패널
	...
	public int mineX[]           = new int[200]; // 지뢰의 X좌표 저장
	public int mineY[]           = new int[200]; //지뢰의 Y좌표 저장
	final int SIZE_OF_PANEL_GARO = 30; //지뢰판 크기 설정
	final int SIZE_OF_PANEL_SERO = 20; // 지뢰판 크기 설정
	final int NUM_OF_MINE        = 120;
	final int OPEN               = 2;
	...
	Timer timer                  = new Timer();
		...
		menuPanel2.add(new JLabel("Lv3 120개"),BorderLayout.CENTER);
		menuPanel2.add(panelLv2,BorderLayout.SOUTH);
    ...
    public void openTile(int x, int y, int GARO, int SERO) {
        ...
        if(x!=0 && 0<=y && y<=GARO-1 && mineRoom[x-1][y] == 0 && openCheck[x-1][y]!=2)
        	openTile(x-1,y,GARO,SERO);
      
        if(x!=SERO-1 && 0<=y && y<=GARO-1 && mineRoom[x+1][y] == 0 && openCheck[x+1][y]!=2)
        	openTile(x+1,y,GARO,SERO);		
    	
        if(y!=0 && 0<=x && x<=SERO-1 && mineRoom[x][y-1] == 0 && openCheck[x][y-1]!=2)
        	openTile(x,y-1,GARO,SERO);
        
        if(y!=GARO-1 && 0<=x && x<=SERO-1 && mineRoom[x][y+1] == 0 && openCheck[x][y+1]!=2)
        	openTile(x,y+1,GARO,SERO);
        
        if(x!=0 && 1<=y && y<=GARO-1 && mineRoom[x-1][y-1] == 0 && openCheck[x-1][y-1]!=2)
        	openTile(x-1,y-1,GARO,SERO);
        
        if(x!=0 && 0<=y && y<=GARO-2 && mineRoom[x-1][y+1] == 0 && openCheck[x-1][y+1]!=2)
        	openTile(x-1,y+1,GARO,SERO);
        
        if(x!=SERO-1 && 1<=y && y<=GARO-1 && mineRoom[x+1][y-1] == 0 && openCheck[x+1][y-1]!=2)
        	openTile(x+1,y-1,GARO,SERO);
        
        if(x!=SERO-1 && 0<=y && y<=GARO-2 && mineRoom[x+1][y+1] == 0 && openCheck[x+1][y+1]!=2)
        	openTile(x+1,y+1,GARO,SERO);
...
MouseListener RightClickListener = new MouseListener() {
	...
	public void saveGame() {
	    ...
	    String mine = "E:\\Temp\\mineRoomLv3.txt";
		String open = "E:\\Temp\\openCheckLv3.txt";
		String click = "E:\\Temp\\clickCheckLv3.txt";
		String timerPath = "E:\\Temp\\timerLv3.txt";
